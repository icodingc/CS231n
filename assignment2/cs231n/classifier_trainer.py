...
# only save best_model
def save_params(iter, params):
    with open("saved_params_%d.npy" % iter, "w") as f:
        pickle.dump(params, f)


#当usesaved=True时，确保load的参数适用于训练的模型
class ClassifierTrainer(object):
  ...
  def train(self, X, y, X_val, y_val, 
            model, loss_function, 
            reg=0.0,dropout=1.0,m='softmax',d = 0.5,
    ...
    """
    if usesaved:
      start_iter,oldw = load_saved_params()
      if start_iter > 0:
        model = oldw
    ...
    for it in xrange(num_iters):
      ...
      ##  can add dropout..
      cost, grads = loss_function(X_batch, model, y_batch, reg, d)
