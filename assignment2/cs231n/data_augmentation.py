...
def random_crops(X, crop_shape):
  ...
  #############################################################################
  for i in xrange(N):
      a = np.random.randint(H - HH)
      b = np.random.randint(W - WW)
      out[i,:,:,:] = X[i,:,a:a+HH,b:b+WW]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
...
##use random_sample vectorization
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  #############################################################################
  # for i in xrange(N):
  #     out[i,:,:,:] = X[i,:,:,:]*np.random.uniform(low,high)
  l = (high-low)*np.random.random_sample((N))+low
...
def random_tint(X, scale=(-10, 10)):
  ...
  # for i in xrange(N):
  #     for j in xrange(C):
  #     out[i,j,:,:] = X[i,j,:,:]+ np.random.uniform(low,high)
  l = (high - low)*np.random.random_sample((N,C)) + low
  # trick:how to transform
  out = X + l[:,:,None,None]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
